...
> fabric budget** on one line: "Order-ID map lookup", marked `fixed`. It is the only place
> in the tick-to-trade path where a *data structure*, not a pipeline stage, owns the
> latency. Everything here defends two properties of that row: 2 cycles for **every** key
> with no probing loop, and a lookup that fails says so loudly instead of quietly returning
> the wrong order. [04.03](../04-system-architecture/03-order-book-in-hardware.md) §2 is the
> design summary; this is the derivation, the mathematics, the tooling and the failure
> analysis.
...
> semantics in [08.04](../08-nasdaq/04-totalview-itch-5.0.md) §3 and §7. The key width sets
> the tag width, which sets the memory — confirm before freezing the entry format.

| # | Requirement | What it forbids |
| --- | --- | --- |
| **R1** | **Fixed latency.** Exactly 2 cycles, every key, hit or miss | Linear probing, chaining, rehash-on-miss — anything whose worst case is a loop |
| **R2** | **Bounded storage**, sized at elaboration (CLAUDE.md §5.1) | Malloc, exhaustible free lists, tree rebalancing |
| **R3** | **No unnoticed miss.** Every failure counted and attributable | "If not found, ignore" — the most expensive line of code available here |
| **R4** | **Read *and* written at line rate** | An unbypassed read-modify-write |

R4 is the one that gets missed. `A` and `D` for the same reference can arrive in adjacent
messages, and `symbol_filter` expands `U` into a delete plus an insert back to back. The
order map has the same RMW hazard as the level array at the same cycle distance and needs
the same write-forwarding bypass — [04.03](../04-system-architecture/03-order-book-in-hardware.md) §8,
...
A CAM compares the key against **every** entry in parallel: `N × K` bits of comparator plus
an `N`-wide match encode, both linear in capacity, with no logarithmic structure to exploit
because the keys are unsorted. At `N = 65,536` and `K = 64`, roughly `N·(K/2) ≈ 2.1 M` LUT6
for the compare alone — about 2× a VU9P's entire LUT supply, for one table.
...
> magnitude, not percent) does not depend on it.

| Structure | Lookup | Insert | Capacity at 65 K × 64-bit keys | Determinism | Verdict |
| --- | --- | --- | --- | --- | --- |
| **LUT/register CAM** | 1–2 cyc | 1 cyc | ~10²–10³ before it dominates the device | Perfect | **Right primitive, wrong scale.** Keep for the 64-entry overflow region (§5) |
| **BRAM-emulated CAM** (bit-sliced, one read per key bit) | **64+ cyc** | many | Large | Fixed, hopeless | 64 cycles is 3× the whole fabric budget |
...
the memory doing the narrowing for free.

---

## 3. Hash function choice in fabric

### 3.1 The requirement, and ⚠️ why it is not a cryptographic hash

A **fixed, combinational, stateless** function of the key that fits the logic and routing
slack of one 6.4 ns cycle, uses zero DSPs, has no state to reset, and spreads the *actual*
observed key distribution over `2^INDEX_W` sets.

SHA-2/3 or an AES mixer costs hundreds of cycles serially, or a huge unrolled pipeline, to
buy collision resistance **against an adversary who chooses keys**. That adversary does not
exist here: the exchange assigns reference numbers, a competitor cannot pick one to collide
with our table and cannot observe the table. We are not buying that property.

⚠️ **That is not "the keys are benign".** They are **structured** — Nasdaq assigns
...
### 3.2 CRC-based XOR trees — the recommended primitive

A CRC is **linear over GF(2)**: an `n`-bit message through a degree-`r` CRC with the
register initialised to zero computes `R(x) = M(x)·x^r mod P(x)`, so by superposition
`CRC(M) = ⊕_{j : M_j = 1} CRC(e_j)`. The function collapses to a **constant GF(2) matrix** —
each output bit is a fixed XOR of a subset of input bits. No shift register, no state, no
cycles: one XOR reduction tree per output bit, all `r` in parallel.

```
Fan-in per output bit F ⇒ ⌈log₆ F⌉ LUT6 levels  (a LUT6 is any 6-input function, so a
                                                 6-input XOR is exactly 1 LUT)
...
Fan-ins computed by the generator in §3.4, not quoted. Three LUT levels plus routing sits
...
### 3.3 ⚠️ The trap: slicing the low bits of a monotonic reference

`set = order_ref[13:0]` is a *perfect* hash for a monotone key: buckets fill round-robin,
the histogram is flat, it beats the CRC on every metric, and it is free. It is also the
worst decision available in this file.
...
Row three is the point: **our own design breaks the assumption, not the venue's.**
...
> incident. [01.03](../01-fpga-design/03-memory-and-storage.md) §7.2, with its failure modes.

| Alternative | LUT levels | Area | Avalanche | Notes |
| --- | ---: | --- | --- | --- |
...
| XOR-fold `ref[63:32] ^ ref[31:0]` | 1 | ~64 LUT | **Poor** — the high half is near-constant, degenerating to the row below | Rejected |
| Bit-slice `ref[13:0]` | 0 | 0 | None | ⚠️ Rejected, per the table above |
...
### 3.4 The generator, and measuring hash quality

The XOR matrix is derived, not written. The same script proves the bijection §4.2 needs.
...
assert gf2_rank(cols, N) == N, "polynomial is NOT a bijection on 64 bits"   # §4.2
for k in range(N):                                # gf2_rank: textbook GF(2) Gaussian elim.
...
Quality is then **measured on real keys**, never asserted:
...
for msg in itch_messages("day.pcap", locates=TRACKED_LOCATES):   # filtered — see §5.4
    if msg.type in b"AF":
        s = crc64(msg.ref, cols) & ((1 << INDEX_W) - 1)          # crc64 = XOR of cols[j]
...
print("peak max load", h.max(), "| sets over W", (h > WAYS).sum(),
      "| excess records", np.maximum(h - WAYS, 0).sum())
```
...
```
   order_ref[63:0] ─► crc_hash (combinational, in the SYMBOL-FILTER stage — §4.3)
...
| `WAYS` | 4 | §7: 8 ways buy ~15× on `P(overflow)` at α = 0.25 but double row width and compare fan-in. 4 is the knee |
| Row width | 4 × 128 = **512 b** | `⌈512/72⌉ = 8` URAM288 wide × `16384/4096 = 4` deep = **32 URAM288** |
| Tag | 50 b (§4.1) or the full 64-bit key | Never a truncated non-invertible tag |
| Payload | `{slot, side, level, qty, epoch, ticket}` | `ticket` is [09.01](01-queue-position-and-fill-probability.md) §4.2 riding along free |

### 4.1 The tag: why 50 bits is *exactly* as safe as 64

If the index were an arbitrary hash, "the remaining key bits" would not be a well-defined
tag and truncation would admit false hits. But a **CRC-64 over a 64-bit input with
...
term. Every standard CRC polynomial does; `gen_hash.py` asserts it. So
`{tag[49:0], set[13:0]} = h[63:0]`, which determines the key uniquely: two distinct
references cannot share both index and tag. **Zero false positives at 50 bits.**
...
| **50, bijective split** / 64, full key | **0** | **0** | **Exact** |

⚠️ A tag false positive is not a miss — it is a **hit on the wrong order**: a delete applied
to a different order, at a different price, in a different symbol, with no counter
incrementing anywhere. At a 128-bit entry the full key already fits, so the honest
recommendation is **store the full key**, and use the bijection argument for a 50-bit tag
only if the width budget forces it.

### 4.2 Hitting 2 cycles, and the mistake that costs a third

The budget row reads *"Order-ID map lookup (BRAM + out reg) — 2 cycles, fixed"*: **one
memory access + one compare/select**. There is no third cycle for the hash.
...
> into the symbol-filter row — one cycle that does almost no work — costs **zero budget rows**.
...
the `fpga_top` budget row describe different configurations; only one of them fits.
...
// Ports elided: clk/rst, {set_q,tag_q} registered from the symbol-filter stage, the
...
Evict the record for order X and the order **still exists at the venue**. Minutes later its
`Order Delete` arrives, you miss, and you cannot apply the delete — you do not know its
price, side or remaining quantity, so there is nothing to subtract. Those shares stay on
that price level **for the rest of the session**. The book now shows liquidity that is not
there, and every quote, imbalance and trigger derived from that level is a fiction.
...
| `omap_overflow_occ` / `_hwm` | Overflow CAM occupancy / high-water | < 50 % of capacity | Table undersized; rebuild with more sets (§7) |
...
⚠️ The two miss counters must be **separate registers**. Merging them is a one-line
"simplification" that destroys the only signal distinguishing normal operation from a
silently diverged book. Semantics per
...
| **Spill to a small fully-associative overflow CAM** | 2 cyc (compared **in parallel**, +0) | Correct, bounded, occupancy is an early warning | **CHOSEN** |
| Overflow full → `book_stale` + resync | 2 cyc | Correct, loud, self-healing | **CHOSEN** terminal case |

64 entries × 64-bit compare is ~600 LUT: §2's rejected primitive, at the scale where it wins.
...
of all live orders. The honest form is
`reduction = (live orders across ALL symbols) / (live orders across OUR 128)`, a **measured**
ratio from §8 on a real day for the actual universe. It is still the largest single lever in
the design — [08.04](../08-nasdaq/04-totalview-itch-5.0.md) §7 is right to call it the most
effective resource optimisation in the feed handler — but quoting 70× will undersize the table.
...
**Cuckoo.** `d` independent hashes, `d` parallel reads, compare all. **Lookup is O(1) worst
case** — exactly the property R1 demands. *Insertion* is the problem: if all `d` candidates
are full you evict an incumbent and re-insert it at *its* alternate location, recursively,
with no useful worst-case bound and the possibility of a cycle. ⚠️ Here the insert path is
**also** critical — `Add Order` arrives at line rate — so an unbounded displacement chain
violates CLAUDE.md §5.2 outright. Bounded form: cap the chain at `K`, on exhaustion spill to
the overflow CAM, and if that is full go `book_stale`. §5.1 applies unchanged — a
displacement that *drops* an incumbent is an eviction under another name.
...
candidate buckets and place into the **least loaded**, ties to the left. One attempt, no
displacement, no chain, no retry; lookup reads all `d` and compares, again fixed. The "power
of `d` choices" is why it works: with one random choice the maximum load over `n` bins grows
like `log n / log log n`; with `d ≥ 2` it collapses to `log log n / log d + O(1)`, and left
tie-breaking improves the constant — an exponential tail improvement for one memory port.
...
its *insert* path, not its lookup path. d-left is better engineering and would win if URAM
were scarce — it is not (32 of ~960), so we buy occupancy with memory rather than with a
second hash, two more memory instances and a min-select on the insert path. **Memory is
cheap here; the 2-cycle row and design simplicity are not.** Revisit if §8 measures a peak
that 65,536 entries cannot hold at α ≤ 0.25.
...
the number §8's measured peak must be checked against. At a peak near 30,000 the overflow
must grow to ~1,300 entries (no longer a register CAM — it becomes a second hashed table),
or `WAYS` must go to 8.
...
Worked, at the design point (every input from §8, none assumed):
  n_peak (measured, §8)  N        safety factor (policy)  × 2   ⇒ design n = 2N
  α = 2N/65,536,  λ = 4α
...
⚠️ The Poisson model assumes a uniform hash, so it is only valid if §3.4's measured
histogram matches it. A measured max load above the prediction indicts the **hash**, not the
sizing.
...
for msg in itch_messages("S050125-v50.pcap"):
    ...
    tot = sum(live_by_sym.values()); hwm_total = max(hwm_total, tot)
...
all day ([08.04](../08-nasdaq/04-totalview-itch-5.0.md) §7). **Keep the time series** — the
aggregate peak is almost certainly just after 09:30 or just before 16:00, the two windows
where an overflowing table is most expensive
([09.08](08-market-open-and-close-dynamics.md)).
...
the universe can grow to hundreds of symbols. The arithmetic kills it: the **entire**
tick-to-trade fabric budget is 128 ns and the lookup row is 12.8 ns, while a PCIe round trip
plus a host DRAM read plus DMA completion is µs-scale — two to three orders of magnitude
over the whole budget, not just the row.
...
([09.07](07-jitter-sources-and-determinism.md)). You cannot predict which orders get
deleted, so you cannot make the hot set the right set.
...
a divergence stales the symbol); rebuilding after a gap, already a slow-path operation
([04.03](../04-system-architecture/03-order-book-in-hardware.md) §9); the §3.4 and §8
analysis; and the regulatory audit trail
([03.06](../03-algotrading/06-risk-and-compliance.md)). Same conclusion as
[04.03](../04-system-architecture/03-order-book-in-hardware.md) §2.6, from the jitter side
rather than the mean-latency side.
...
| **No false hit** | Insert `n` keys, then look up `n` keys **known absent**, chosen to collide in the index bits | `rd_hit` never asserts — the §4.1 property under test |
| **No lost record** | Insert `n` keys, look up all `n` shuffled | Every one hits. Any loss ⇒ eviction or bypass bug |
| **RMW hazard (R4)** | Add then Delete for the same ref at cycle distance 1, 2, 3; Add/Add into one set; `U` expansion | Delete hits; `omap_bypass_hit` increments. ⚠️ A distance-3-only test passes with no bypass at all |
| **Overflow path + exhaustion** | Fill a set to `WAYS` and insert a 5th; then fill the overflow CAM and insert once more | 5th hits via the CAM at the **same 2-cycle latency**; then `omap_insert_fail` + `book_stale`, **no eviction**, no wrong-order delete |
...
| **Hash matrix + quality** | Generated `crc_hash.sv` vs a bit-serial CRC over 10⁶ random keys; then §3.4 over a full replayed day | Bit-exact and `gf2_rank == 64`; measured max load ≤ the §7 Poisson prediction (exceeding it indicts the hash, not the sizing) |
